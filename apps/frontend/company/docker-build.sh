...
# Script de build Docker pour SkillForge AI Company App
...
build_image() {
    ...
    # Build de l'image
    docker build \
        -f "apps/frontend/company/$DOCKERFILE" \
        -t "$image_name" \
        --build-arg BUILD_DATE=$(date -u +'%Y-%m-%dT%H:%M:%SZ') \
        --build-arg VCS_REF=$(git rev-parse --short HEAD 2>/dev/null || echo "unknown") \
        --build-arg VERSION=$TAG \
        apps/frontend/company/
...
# Fonction principale
main() {
    log_info "Starting Docker build for SkillForge AI Company App"
