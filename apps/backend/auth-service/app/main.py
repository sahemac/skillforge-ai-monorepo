...
async def lifespan(app: FastAPI):
    ...
    # Startup
    logger.info("Starting SkillForge AI Auth Service...")
    ...
    # Shutdown
    logger.info("Shutting down SkillForge AI Auth Service...")
...
# Create FastAPI app
app = FastAPI(
    title="SkillForge AI - Auth Service",
    description="Microservice for authentication and authorization in SkillForge AI platform",
    version="1.0.0",
...
# IMPORTANT: Middleware order - add from outermost to innermost
# CORS must be FIRST middleware added to wrap all others

# CORS middleware - add FIRST to intercept all requests/responses
# Temporary: use wildcard origin without credentials for testing
app.add_middleware(
    CORSMiddleware,
    allow_origins=["*"],
    allow_credentials=False,  # Must be False to use wildcard
    allow_methods=["*"],
    allow_headers=["*"],
)

# TrustedHostMiddleware - disabled in development to avoid port conflicts
...
# Add IAP middleware (executes closest to route handlers)
app.add_middleware(
    IAPMiddleware,
    project_number="584748485117",
    backend_service_id="auth-service-backend-staging"
)
...
async def root():
    ...
    if os.path.exists(login_file):
        ...
        return JSONResponse({
            "service": "skillforge-ai-auth-service",
            "version": "1.0.0",
            "status": "healthy",
