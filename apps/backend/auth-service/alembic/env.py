...
# Auth-service OWNS: user_two_factor table (READ/WRITE)
# Auth-service READS: users table (READ-ONLY, for foreign keys)
from skillforge_models import User, UserTwoFactor
...
def get_database_url():
    ...
    # 4. Erreur si pas de configuration
    raise ValueError("DATABASE_URL or POSTGRES_PASSWORD must be set for auth-service")
...
def run_migrations_offline() -> None:
    ...
    context.configure(
        ...
        # Only include tables owned by auth-service
        include_schemas=True,
        include_object=lambda obj, name, type_, reflected, compare_to: (
            # Include user_two_factor table (owned by auth-service)
            name == "user_two_factor" if type_ == "table" else True
        ),
    )
