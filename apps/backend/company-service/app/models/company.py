"""
Company Profile model for SkillForge AI Company Service
"""
...
class CompanyProfile(SQLModel, UUIDMixin, TimestampMixin, table=True):
    ...
    # Owner Information (just the UUID, no relationship)
...
class CompanyTeamMember(SQLModel, UUIDMixin, TimestampMixin, table=True):
    ...
    user_id: uuid.UUID = Field(nullable=False, index=True)  # No foreign key to users table
    ...
    invited_by: Optional[uuid.UUID] = Field(default=None)  # No foreign key to users table
