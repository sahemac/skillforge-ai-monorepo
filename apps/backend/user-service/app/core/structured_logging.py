"""
Structured logging configuration for SkillForge AI
...
def log_database_operation(operation: str, table: str, success: bool, duration_ms: float, error: Optional[str] = None):
...
def log_authentication_event(event_type: str, user_id: Optional[str] = None, success: bool = True, error: Optional[str] = None):
    ...
    if error:
        context['error'] = error
        logger.warning("Authentication event failed", **context)
    else:
        logger.info("Authentication event completed", **context)
...
def log_security_event(event_type: str, severity: str = 'medium', details: Optional[Dict[str, Any]] = None):
    """Log security events."""
    ...
    if severity == 'high':
        logger.error("High severity security event", **context)
    elif severity == 'medium':
        logger.warning("Medium severity security event", **context)
    else:
        logger.info("Low severity security event", **context)


def log_performance_metric(metric_name: str, value: float, unit: str = 'ms', metadata: Optional[Dict[str, Any]] = None):
    ...
    logger.info("Performance metric recorded", **context)
