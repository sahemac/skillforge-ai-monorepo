"""
Base CRUD operations for SkillForge AI Project Service
"""
...
class CRUDBase(Generic[ModelType, CreateSchemaType, UpdateSchemaType]):
    ...
    async def restore(self, db: AsyncSession, id: UUID) -> Optional[ModelType]:
        ...
        db_obj = await self.get(db, id, include_inactive=True)
        if db_obj and hasattr(db_obj, "is_active"):
            update_data = {
                "is_active": True,
                "updated_at": datetime.utcnow()
    ...
    async def exists(self, db: AsyncSession, id: UUID, include_inactive: bool = False) -> bool:
    ...
    async def get_paginated(
        ...
        # Get total count
        total = await self.count(db, filters=filters, include_inactive=include_inactive)
            ...
            if hasattr(self.model, field_name) and field_value is not None:
                ...
                if isinstance(field_value, list):
                    ...
                    if "gte" in field_value:
                        query = query.where(field >= field_value["gte"])
                    if "lte" in field_value:
                        query = query.where(field <= field_value["lte"])
                    if "gt" in field_value:
                        query = query.where(field > field_value["gt"])
                    if "lt" in field_value:
                        query = query.where(field < field_value["lt"])
                    if "like" in field_value:
                        query = query.where(field.ilike(f"%{field_value['like']}%"))
                    if "not_in" in field_value:
                        query = query.where(~field.in_(field_value["not_in"]))
                else:
